 ...
 * 将多条教育路径聚类为战略路线
 */
export class PathClustererService {
   ...
   * 计算转轨时机标签
   */
  private calculateTransTimingTag(path: EducationPath): TransTimingTag {
    const internationalStages: string[] = [];
   ...
   * 判断是否为国际体系水平
   */
  private isInternationalLevel(stage: string, level: string): boolean {
    switch (stage) {
      case '小学':
      case '初中':
   ...
   * 计算目标学位标签
   */
  private calculateTargetDegreeTag(userInput: UserInput): TargetDegreeTag {
   ...
   * 计算学位类型标签
   */
  private calculateDegreeTypeTag(path: EducationPath, userInput: UserInput): DegreeTypeTag {
    // 找到目标阶段的节点
   ...
   * 计算可行性标签
   */
  private calculateFeasibilityTag(path: EducationPath): FeasibilityTag {
    // 检查路径中所有节点的可行性
   ...
   * 计算路径的核心标签
   */
  private calculatePathCoreTags(path: EducationPath, userInput: UserInput): PathCoreTags {
    return {
   ...
   * 计算路径的常见度
   */
  private calculatePathPrevalence(path: EducationPath): number {
    // 基于特征组合计算常见度
   ...
   * 将路径聚类为战略路线
   */
  clusterPaths(paths: EducationPath[], userInput: UserInput): StrategicRoute[] {
   ...
   * 根据目标阶段获取学位名称
   */
  private getDegreeName(targetStage: string): string {
    switch (targetStage) {
      case '大学':
        return '本科';
      case '研究生':
   ...
   * 计算可行性评分
   */
  private calculateFeasibilityScore(paths: EducationPath[]): number {
   ...
   * 计算费用区间
   */
  private calculateCostRange(paths: EducationPath[]): { min: number; max: number } {
    const costs = paths
   ...
   * 计算战略路线的常见度
   */
  private calculateRoutePrevalence(paths: EducationPath[]): number {
    if (paths.length === 0) return 0;
