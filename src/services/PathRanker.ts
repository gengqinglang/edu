import { EducationPath, RankedPath, RankingWeights, PathFeature, PathCoreTags, EducationLevel } from '../types';
import { EDUCATION_TRANSITION_RULES } from '../../EDUCATION_TRANSITION_RULES';
import { MatchScoreCalculatorService } from './MatchScoreCalculator';

/**
 * 路径排序服务
 * 根据多个维度对路径进行智能排序
 */
export class PathRankerService {
  ...
  private matchScoreCalculator: MatchScoreCalculatorService;
   ...
   * 对路径进行排序
   */
  rankPaths(paths: EducationPath[], weights?: Partial<RankingWeights>): RankedPath[] {
    ...
    const rankedPaths = paths.map(path => {
      ...
      return {
        ...
        rankingReasons,
        features,
        coreTags: {} as PathCoreTags // 临时占位，实际由PathClusterer计算
      };
    });
   ...
   * @returns 排序后的路径列表
   */
  rankPathsByCurrentState(paths: EducationPath[], currentLevel: EducationLevel): RankedPath[] {
    const rankedPaths = paths.map(path => {
   ...
   * 按可行性排序路径
   */
  private sortPathsByFeasibility(paths: RankedPath[]): RankedPath[] {
   ...
   * 计算路径的排序分数
   */
  private calculateRankingScore(path: EducationPath, weights: RankingWeights): number {
    let score = 0;
   ...
   * 计算可行性分数
   */
  private calculateFeasibilityScore(path: EducationPath): number {
   ...
   * 计算常见度分数
   */
  private calculatePrevalenceScore(path: EducationPath): number {
    ...
    for (let i = 0; i < path.nodes.length - 1; i++) {
      ...
      const rule = EDUCATION_TRANSITION_RULES.find(r => 
   ...
   * 计算费用分数
   */
  private calculateCostScore(path: EducationPath): number {
   ...
   * 计算转轨次数分数
   */
  private calculateTransitionScore(path: EducationPath): number {
   ...
   * 判断是否为转轨
   */
  private isTransition(fromLevel: string, toLevel: string): boolean {
   ...
   * 分析路径特征
   */
  private analyzePathFeatures(path: EducationPath): PathFeature[] {
   ...
   * 生成排序原因
   */
  private generateRankingReasons(path: EducationPath, weights: RankingWeights): string[] {
   ...
   * 计算转轨次数
   */
  private countTransitions(path: EducationPath): number {
    let count = 0;
